...
HuaweiR4850Component::HuaweiR4850Component(canbus::Canbus *canbus) {
  ESP_LOGD(TAG, "Constructor called");
  this->canbus = canbus;
}
  ...
  if (can_id == CAN_ID_DATA) {
    ...
    switch (data[1]) {
      ...
      case R48xx_DATA_OUTPUT_CURRENT_MAX:
        ...
        ESP_LOGD(TAG, "Max Output current: %f", conv_value);
        break;
